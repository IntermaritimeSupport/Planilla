...
export const AllLegalParameters: React.FC = () => {
  ...
  useEffect(() => {
    authFetcher<{ key: string; name: string; category: string }[]>(
      `${API_URL}/api/system/legal-parameters/keys`
    )
      .then(data => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEditing) {
        await apiPut(`/api/system/legal-parameters/${modal.parameter?.id}`, payload)
      } else {
        await apiPost(`/api/system/legal-parameters`, payload)
      }
  ...
  const deleteParameter = async (id: string) => {
    ...
    try {
      await apiDelete(`/api/system/legal-parameters/${id}`)
